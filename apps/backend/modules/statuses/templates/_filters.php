...
}}" action="<?php echo url_for('statuses_collection', array('action' => 'filter')) ?>" method="post">
    <table cellspacing="0">
      <tfoot>
        <tr>
          <td colspan="2">
            <?php echo $form->renderHiddenFields() ?>
            <?php /*echo link_to('Resetear', 'statuses_collection', array('action' => 'filter'), array('query_string' => '_reset', 'method' => 'post'))*/ ?>
            <input type="submit" value="<?php echo 'Buscar'; ?>" />
            <a class="boton cancelar" href="<?php echo url_for('statuses/index'); ?>" onclick ="javascript:window.close();" >Cerrar</a></ul>          </td>
        </tr>
      </tfoot>
      <tbody>
        ...
        <?php if ((isset($form[$name]) && $form[$name]->isHidden()) || (!isset($form[$name]) && $field->isReal())) continue ?>                                                                                                                
          <?php include_partial('statuses/filters_field', array(
            'name'       => $name,
